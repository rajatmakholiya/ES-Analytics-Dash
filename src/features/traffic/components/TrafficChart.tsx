    ...
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3 rounded-lg shadow-lg">
        ...
        {payload.map((entry: any, index: number) => (
          <div key={index} className="flex items-center gap-2 text-xs font-medium mb-1" style={{ color: entry.color }}>
...
export function TrafficChart({ data, dateHeaders, stats }: TrafficChartProps) {
  const chartData = useMemo(() => {
    ...
    [...dateHeaders].reverse().forEach(date => {
      dailyMap[date] = { name: format(parseISO(date), 'MMM dd'), sessions: 0, users: 0 };
    });
    data.forEach(page => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <Legend />
              <Area type="monotone" dataKey="sessions" stroke="#2563eb" strokeWidth={2} fill="url(#colorSessions)" />
              <Area type="monotone" dataKey="users" stroke="#8b5cf6" strokeWidth={2} fillOpacity={0} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 flex flex-col">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Metric Distribution</h3>
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-6">Ratio of Sessions vs Users vs Views</p>
        <div className="flex-1 min-h-[250px] relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
            <span className="text-3xl font-bold text-gray-900 dark:text-white">{stats.sessions.toLocaleString()}</span>
            <span className="text-xs text-gray-400 uppercase font-semibold">Sessions</span>
          </div>
        </div>
      </div>
    </div>
